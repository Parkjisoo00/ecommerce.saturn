...
COMMENT ON COLUMN "TB_BUY_DTL"."CD_STATE" IS '구매 상품 상태 : 결제 실패(NULL),1(결제), 2(취소), 3(교환), 4(환불)';

COMMENT ON COLUMN "TB_BUY_DTL"."CD_STATE_DELIVERY" IS '배송 상태 코드: 결제 실패(NULL), 판매 확인중(C), 배송 준비중(P), 배송중(D), 배송 완료(Y)';
